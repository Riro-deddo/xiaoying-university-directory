...
function validateAnomaly(anomaly) {
  ...
  if (anomaly.missingRequiredText !== undefined && (
    !Array.isArray(anomaly.missingRequiredText)
    || anomaly.missingRequiredText.some((text) => typeof text !== 'string' || !text.trim())
  )) {
    throw new TypeError('missingRequiredText must contain non-empty strings');
  }
...
上一版公开状态已保留；自动巡查不会替换正式课程入口或改写课程数据。

请人工检查官网入口、跳转结果或页面身份锚点，确认后再更新受审数据。本 Issue 不代表课程申请资格结论。
`;
  }
  return `<!-- source-anomaly:${anomaly.sourceId} -->
## 官方来源自动同步异常
