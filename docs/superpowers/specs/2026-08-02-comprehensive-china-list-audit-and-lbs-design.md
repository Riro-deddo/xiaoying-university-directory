...
公开名单可能包含数千行，默认折叠不能减少首屏下载体积。因此首页只输出来源、规则、数量和静态数据地址；用户第一次展开某所大学时，浏览器才读取该来源对应的静态 JSON 并渲染院校行。每个来源单独生成一个 JSON 文件，浏览器可正常缓存，失败时保留规则说明和官网链接并显示“名单暂时无法加载”。这仍然是 GitHub Pages 静态托管，不增加服务器、数据库或运行费用。

中国本科院校反向查询数据同样不进入首屏脚本；只有用户切换到反向查询模式时才加载静态反向索引。静态数据地址必须使用 Astro 的站点 base path，保证本地 `/` 和 GitHub Pages `/xiaoying-university-directory/` 都能访问。
...
- 体积测试：首页 HTML 不包含数千条院校行；每个来源生成独立静态 JSON，反向索引仅在切换模式后加载。
